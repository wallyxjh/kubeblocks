...
# Deliberately make the non-leader target a non-RO member. A target which is
# already leader is an idempotent no-op, so it cannot exercise role rejection.
set_status rw-0 RW RW
apply_op contract-test-switchover-reject.yaml polardb-stack-contract-switchover-reject
wait_for_op polardb-stack-contract-switchover-reject Failed
switch_annotation="$(kubectl get mpdcluster "${MPD_CLUSTER}" -n "${NAMESPACE}" \
  -o jsonpath='{.metadata.annotations.switchRw}')"
[[ -z "${switch_annotation}" ]] || {
  echo "RW switchover rejection unexpectedly wrote switchRw=${switch_annotation}" >&2
  exit 1
}
...
echo "PolarDB Stack Ops KB 0.8 API-contract test passed (no Stack Operator or physical STONITH was exercised)"
