...
ops_identity="${KB_OPS_NAMESPACE}/${KB_OPS_NAME}"
payload="{\"cluster\":\"${MPD_CLUSTER}\",\"namespace\":\"${MPD_NAMESPACE}\",\"targetInstance\":\"${TARGET_INSTANCE}\",\"reason\":\"${reason}\",\"opsRequest\":\"${ops_identity}\"}"
...
mpd_annotate "polardb-pg.kubeblocks.io/last-stonith=${ops_identity}"
